...
const OTPVerification = ({route}: OTPVerificationProps) => {
  ...
  const verifyOpt = async () => {
    ...
    try {
      ...
      if (
        response.data.sCode === 2 &&
        response.data.message === 'No registered user(s) are found.'
      ) {
        alert('Contact the Admin for registration.');
        return;
      }
      ...
      await AsyncStorage.setItem(
        'users',
        JSON.stringify(response.data.resData.users),
      );
      const users = await JSON.parse(await AsyncStorage.getItem('users'));
      console.log(users);
  ...
  const retrievingData = async () => {
    try {
      ...
      if (numberOfTimeResendPressed !== null && oldNumber !== null) {
        if (Number !== oldNumber) {
          await AsyncStorage.setItem('Number', JSON.stringify(0));
        } else if (numberOfTimeResendPressed > 3) {
          if (otpBannedTime === null) {
            await AsyncStorage.setItem(
              'otpBannedTime',
  ...
  const reSendOTP = async () => {
    ...
    if (numberOfTimeResendPressed > 3) {
      alert(
